 ...
 *
 * Capability: robo2mqtt/{robotId}/event/media/offer
 */
class Robo2mqttRobotIdEventMediaOfferCapability(
    ...
    fun handleCommand(command: String, payload: String?) {
        when (command) {
            else -> mqtt.publishError("Unknown command '$command' in robo2mqtt/{robotId}/event/media/offer")
        }
    }
}
