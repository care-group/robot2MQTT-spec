 ...
 *
 * Capability: robo2mqtt/{robotId}/event/media/ice
 */
class Robo2mqttRobotIdEventMediaIceCapability(
    ...
    fun handleCommand(command: String, payload: String?) {
        when (command) {
            else -> mqtt.publishError("Unknown command '$command' in robo2mqtt/{robotId}/event/media/ice")
        }
    }
}
