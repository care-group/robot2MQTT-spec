 ...
 *
 * Capability: robo2mqtt/{robotId}/event/movement/status
 */
class Robo2mqttRobotIdEventMovementStatusCapability(
    ...
    fun handleCommand(command: String, payload: String?) {
        when (command) {
            else -> mqtt.publishError("Unknown command '$command' in robo2mqtt/{robotId}/event/movement/status")
        }
    }
}
