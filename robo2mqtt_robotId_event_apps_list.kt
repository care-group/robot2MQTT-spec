 ...
 *
 * Capability: robo2mqtt/{robotId}/event/apps/list
 */
class Robo2mqttRobotIdEventAppsListCapability(
    ...
    fun handleCommand(command: String, payload: String?) {
        when (command) {
            else -> mqtt.publishError("Unknown command '$command' in robo2mqtt/{robotId}/event/apps/list")
        }
    }
}
