 ...
 *
 * Capability: robo2mqtt/{robotId}/cmd/apps/start
 */
class Robo2mqttRobotIdCmdAppsStartCapability(
    ...
    fun handleCommand(command: String, payload: String?) {
        when (command) {
        "start" -> client.onStart(payload)
            else -> mqtt.publishError("Unknown command '$command' in robo2mqtt/{robotId}/cmd/apps/start")
        }
    }
}
